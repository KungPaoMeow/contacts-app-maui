...
//app.UseHttpsRedirection();    // https makes the android emulator not work with current settings


// Define API routes
app.MapGet("/api/contacts", async ([FromQuery]string? filter, ApplicationDbContext db) =>
{
    ...
    if (string.IsNullOrWhiteSpace(filter))
        ...
        contacts = await db.Contacts.Where(x =>
            ...
            !string.IsNullOrWhiteSpace(x.Address) && x.Address.ToLower().IndexOf(filter.ToLower()) >= 0 ||
            !string.IsNullOrWhiteSpace(x.Phone) && x.Phone.ToLower().IndexOf(filter.ToLower()) >= 0).ToListAsync();
